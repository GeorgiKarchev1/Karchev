    ...
    <footer className="studio-footer">
      <div className="studio-wrap">
        <div className="studio-footer-top">
          ...
          <div><h2>{bg ? 'За бизнеса Ви' : 'For your business'}</h2><Link href={'/' + locale + '#solutions'}>{bg ? 'Как работим заедно' : 'Working together'}</Link><Link href={'/' + locale + '#how-it-works'}>{bg ? 'Процес' : 'Process'}</Link><a href={bookingPath(bg)}>{bg ? 'Да поговорим' : 'Let’s talk'}</a></div>
