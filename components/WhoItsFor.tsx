...
const getPortfolioSites = (isBG: boolean) => [
  { name: 'GBGamingHub',       url: 'https://www.gbgaminghub.com/',   desc: isBG ? 'Gaming платформа с общност, новини и ревюта за геймъри' : 'Gaming platform with community, news and reviews for gamers',           tag: isBG ? 'Уеб Разработка' : 'Web Development',     img: '/img/gbgaminghub.png' },
  { name: 'The Agency Course',  url: 'https://theagencycourse.bg/',    desc: isBG ? 'Онлайн курс за изграждане на агенция за видео монтаж' : 'Online course for building a video editing agency',             tag: isBG ? 'Уеб Разработка' : 'Web Development',     img: '/img/theagency.png' },
  { name: 'Editing.bg',         url: 'https://editing.bg/',            desc: isBG ? 'Пазар за видео монтажори — автоматизиран от А до Я' : 'Marketplace for video editors — automated from A to Z',               tag: isBG ? 'Уеб & Автоматизация' : 'Web & Automation', img: '/img/editingbg.png' },
  { name: 'InPlayGear',         url: 'https://inplaygear.com/',        desc: isBG ? 'Онлайн магазин за спортна и outdoor екипировка' : 'Online store for sports and outdoor equipment',                  tag: 'E-commerce',          img: '/img/inplaygear.png' },
...
export default function WhoItsFor() {
  ...
  return (
    <section id="portfolio" className="py-20 md:py-32 bg-[#f1f0ea] overflow-hidden">
        ...
        <div className="mb-16 flex flex-col md:flex-row md:items-end justify-between gap-8">
          <div>
            ...
            <h2 className="text-4xl md:text-6xl font-bold text-[#2d232e]">
              {t('portfolio.titleP1')}{' '}
              <span className="border-b-2 border-[#534b52] pb-1">{t('portfolio.titleHighlight')}</span>
            </h2>
          </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {portfolioSites.map((site, index) => (
            <motion.a
              ...
              whileHover={{ y: -12, rotateX: -6, rotateY: index % 2 === 0 ? 4 : -4, scale: 1.03 }}
            ...
            >
              <div className="relative w-full aspect-video overflow-hidden bg-[#f1f0ea]">
                ...
                <div className="absolute inset-0 bg-[#f1f0ea]/50 opacity-0 group-hover:opacity-100 transition-opacity duration-300 flex items-center justify-center">
                  <div className="flex items-center gap-2 text-[#2d232e] font-semibold text-sm bg-[#e0ddcf]/80 backdrop-blur-sm px-4 py-2 rounded-full border border-[#534b52]/30">
                    {t('portfolio.viewSite')} <ArrowUpRight className="w-4 h-4" />
                  </div>
                </div>
              </div>
              ...
              <div className="p-6">
                ...
                <div className="flex items-start justify-between gap-3">
                  <div>
                    <h3 className="text-base font-bold text-[#2d232e] mb-1">{site.name}</h3>
                    <p className="text-[#2d232e] text-sm leading-relaxed">{site.desc}</p>
                  </div>
