...
const content = {
    ...
    sections: [
      {
        ...
        body: 'Ако потърсиш в Google, ще видиш оферти от 150€ до 5000€ за "сайт за малък бизнес." Това не е измама — просто не се говори за едно и също нещо. Разликата е като между ръчно ушита риза и такава от Zara. И двете са ризи. Но не са едно и също.',
      },
      {
  ...
  EN: {
    ...
    readTime: '5 min read',
    date: 'April 25, 2026',
    title: 'How Much Does a Website Cost for a Small Business in Bulgaria — an Honest Answer',
    intro: 'Every time someone asks us "how much does a website cost?" we know what follows — either we give an honest answer and look more expensive than the agency next door, or we say "it depends" and sound like everyone else. We chose the honest answer.',
    sections: [
      {
...
export default function BlogPostClient() {
  ...
  useEffect(() => {
    ...
    if (userPref === 'EN' || userPref === 'BG') { setLang(userPref); return }
  ...
  const articleSchema = {
    ...
    keywords: 'уебсайт цена, колко струва сайт, website cost small business bulgaria',
    ...
    <main className="min-h-screen bg-[#f1f0ea] text-[#2d232e]">
      ...
      <article className="pt-36 pb-24 px-6 md:px-8 max-w-[760px] mx-auto">
        ...
        <div className="flex items-center gap-3 mb-6 flex-wrap">
          ...
          <span className="text-xs text-[#2d232e]/40 font-medium">{c.date}</span>
          <span className="text-xs text-[#2d232e]/40 font-medium">·</span>
          <span className="text-xs text-[#2d232e]/40 font-medium">{c.readTime}</span>
        </div>
              ...
              {'subsections' in section && section.subsections && (
                <div className="space-y-6 mt-2">
                  {section.subsections.map((sub, j) => (
                    <div key={j} className="pl-5 border-l-4 border-[#534b52]/30">
              ...
              {'list' in section && section.list && (
                <ul className="mt-3 space-y-3">
                  {section.list.map((item, j) => (
                    <li key={j} className="flex items-start gap-3 text-base leading-relaxed text-[#2d232e]/75">
